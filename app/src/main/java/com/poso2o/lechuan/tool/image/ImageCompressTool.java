package com.poso2o.lechuan.tool.image;
...
/**
 * Created by mr Zhang on 17/12/26.
 *
 * 图片压缩上传类
 */
public class ImageCompressTool {
    private static ImageCompressTool instance = null;
    ...
    private ImageCompressTool(Context context) {
        this.context = context;
    }

    public static ImageCompressTool getInstance(Context context) {
        if (instance == null) {
            synchronized (ImageCompressTool.class) {
                if (instance == null)
                    instance = new ImageCompressTool(context.getApplicationContext());
            }
        }
        return instance;
    }

    public ImageCompressTool withListener(CompressListener compressListener) {
    ...
    private String getOutputFileName(String srcFilePath) {
        ...
        File file = new File(Environment.getExternalStorageDirectory().getPath(), "Lechuan/Images");
        if (!file.exists()) {
            file.mkdirs();
        }
