...
public class PosterCommentDataManager<T> extends BaseManager {
    public static PosterCommentDataManager mPosterDataManager;
    /**
     * 获取广告评论列表
     */
    private final int TAG_POSTER_ID = 1;

    public static PosterCommentDataManager getPosterDataManager() {
        if (mPosterDataManager == null) {
            mPosterDataManager = new PosterCommentDataManager();
        }
     ...
     * @param callBack       回调
     */
    public void loadPosterCommentData(final BaseActivity baseActivity, final String news_id, final String currPage, final IRequestCallBack callBack) {
        Request<String> request = getStringRequest(HttpAPI.POSTER_COMMENTS_API);
        ...
        baseActivity.request(TAG_POSTER_ID, request, new HttpListener<String>() {
            ...
            public void onSucceed(int what, String response) {
                ...
                Gson gson = new Gson();
                PosterCommentsQueryDTO posterQueryDTO = gson.fromJson(response, PosterCommentsQueryDTO.class);
     ...
     * @param callBack       回调
     */
    public void addPosterComment(final BaseActivity baseActivity, final String news_id, final String comments, final IRequestCallBack callBack) {
        Request<String> request = getStringRequest(HttpAPI.POSTER_ADD_COMMENTS_API);
        request = defaultParam(request);
        //request.add("uid", "13423678930");
        //request.add("token", "e41a8183db9a8b3b650a11846e7b5fc6");
        request.add("news_id", news_id);
