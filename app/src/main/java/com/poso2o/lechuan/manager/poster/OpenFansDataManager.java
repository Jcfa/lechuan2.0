...
public class OpenFansDataManager<T> extends BaseManager {
    public static OpenFansDataManager mOpenFansDataManager;
    /**
     * 获取广告 - 我的粉丝
     */
    private final int TAG_POSTER_ID = 1;

    public static OpenFansDataManager getOpenFansDataManager() {
        if (mOpenFansDataManager == null) {
            mOpenFansDataManager = new OpenFansDataManager();
        }
        return mOpenFansDataManager;
    }
     ...
     * @param callBack       回调
     */
    public void loadOpenFansData(final BaseActivity baseActivity, final String currPage, final String open_uid, final IRequestCallBack callBack) {
        Request<String> request = getStringRequest(HttpAPI.POSTER_OPEN_FANS_API);
