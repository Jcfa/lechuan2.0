...
public class CalendarCard extends View {
	...
	private Paint mSetPaint; //绘制选中日期圆形的画笔
	...
	public CalendarCard(Context context, OnCellClickListener listener,int year,int month,int day) {
		super(context);
		this.mCellClickListener = listener;
		this.year = year;
		this.month = month;
	...
	private void init(Context context) {
		...
		mSetPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
		mSetPaint.setStyle(Paint.Style.FILL);
		mSetPaint.setColor(Color.parseColor("#00BCB4"));//青蓝色
		...
		for (int j = 0; j < TOTAL_ROW; j++) {
			...
			for (int i = 0; i < TOTAL_COL; i++) {
				...
				if (position >= firstDayWeek
					...
					}else if (mShowDate.getYear() == this.year && mShowDate.getMonth() == this.month && day == this.day ){
						//选中日期
						CustomDate date = CustomDate.modifiDayForObject(mShowDate, day);
						rows[j].cells[i] = new Cell(date, State.SETDAY, i, j);
					}
...
//				int row = (int) (mDownY / mCellSpace);
				int col = (int) (mDownX / mCellW);
				int row = (int) (mDownY / mCellH);
	...
	class Cell {
		...
		public void drawSelf(Canvas canvas) {
			...
			case TODAY: // 今天
				...
				canvas.drawCircle( (mCellW*(i + 1) - mCellW/2),
						 (mCellH*(j + 1) - mCellH*3/8), mCellSpace / 3,
						mCirclePaint);
				canvas.drawText("今",
						(mCellW*(i + 1) - mCellW/2 - mTextPaint.measureText("今") / 2), (mCellH*(j + 1) - mCellH/4), mTextPaint);
				return;
			...
			canvas.drawText(content,
					(mCellW*(i + 1) - mCellW/2 - mTextPaint
							.measureText(content) / 2), (mCellH*(j + 1) - mCellH/4), mTextPaint);
		}
	}
	 ...
	 */
	enum State {
		TODAY,CURRENT_MONTH_DAY, PAST_MONTH_DAY, NEXT_MONTH_DAY, UNREACH_DAY,SETDAY;
	}
