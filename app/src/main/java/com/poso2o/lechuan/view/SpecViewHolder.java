...
public class SpecViewHolder {
    ...
    public SpecViewHolder(EditGoodsActivity context, GoodsSpec spec, boolean isVdian) {
    ...
    public SpecViewHolder(EditGoodsActivity context, OldSpec spec, boolean isVdian) {
        this.context = context;
        oldSpec = spec;
        this.isVdian = isVdian;
        this.view = View.inflate(context, R.layout.item_edit_goods_spec_view, null);
...
//            }
            ...
            spec_item_spec.setText(oldSpec.colorid + "/" + oldSpec.sizeid);
            spec_item_stock.setText(NumberFormatUtils.formatToInteger(oldSpec.num));
            spec_item_barcode.setText(TextUtil.trimToEmpty(oldSpec.barcode));
        }
    ...
    private void initListener() {
        ...
        spec_item_to_bar_code.setOnClickListener(new NoDoubleClickListener() {
            ...
            public void onNoDoubleClick(View v) {
                context.toScanCodeActivity(SpecViewHolder.this);
            }
        });
        ...
        spec_item_barcode.addTextChangedListener(new CustomTextWatcher() {
            ...
            public void input(String s, int start, int before, int count) {
                if (isVdian){
                    ...
                    if (spec_item_barcode.getText().toString().trim().length() > 20) {
                        spec_item_barcode.setText(oldSpec.barcode);
                        Toast.show(context, "条码不能超过20个字符");
                    } else {
                        oldSpec.barcode = spec_item_barcode.getText().toString().trim();
                    }
                }
            }
        });
        ...
        spec_item_stock.addTextChangedListener(new CustomTextWatcher() {
            ...
            public void input(String s, int start, int before, int count) {
                if (isVdian){
                    ...
                    oldSpec.num = NumberUtils.toInt(s) + "";
                }
