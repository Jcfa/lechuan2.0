...
//    private ArrayList<Catalog> backData;
    ...
    public void initData() {
        ...
        catalogAdapter = new CatalogAdapter(context, true);
    ...
    private void showDeleteDialog(int position) {
        ...
        // 删除目录分组
        catalogDeleteDialog = new CatalogDeleteDialog(context, true, new CatalogDeleteDialog.OnGroupDelete() {
    ...
    private void loadData() {
        VdianCatalogManager.getInstance().loadList(this, new IRequestCallBack<CatalogBean>() {
            ...
            public void onResult(int tag, CatalogBean result) {
                catalogAdapter.notifyDataSetChanged(result.list);
    ...
    private void showAddDialog() {
        editCatalogDialog = new EditCatalogDialog(context, true, new EditCatalogDialog.OnGroupEditListener() {
    ...
    private void add(Catalog catalog) {
        showLoading("正在提交数据...");
        VdianCatalogManager.getInstance().add(this, catalog, new IRequestCallBack<Catalog>() {
            ...
            @Override
            public void onFailed(int tag, String msg) {
                Toast.show(CatalogActivity.this, msg);
                dismissLoading();
            }
        });
    }
    ...
    private void update(final Catalog catalog) {
        showLoading();
        VdianCatalogManager.getInstance().edit(this, catalog, new IRequestCallBack<Catalog>() {
    ...
    private void delete(final Catalog catalog) {
        showLoading("正在删除目录...");
        VdianCatalogManager.getInstance().del(this, catalog.catalog_id, new IRequestCallBack() {

            @Override
            public void onResult(int tag, Object object) {
                catalogAdapter.deleteItemData(catalog);
                EventBus.getDefault().post(new EventBean(EventBean.CODE_CATALOG_DEL));
                dismissLoading();
            }
    ...
    public void catalogSort() {
        showLoading();
        VdianCatalogManager.getInstance().sort(this, catalogAdapter.getItems(), new IRequestCallBack<String>(){
