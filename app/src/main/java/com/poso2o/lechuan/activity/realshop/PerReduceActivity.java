...
public class PerReduceActivity extends BaseActivity implements View.OnClickListener {
...
//        sharedPreferencesUtil = new SharedPreferencesUtil(context, LoginMemory.LOGIN_INFO);
    ...
    //初始化第一个满减布局
    private void initFirstReduce() {
        perReduceData = new PerReduceData();
        PromotionDetailData firstDetailData = new PromotionDetailData();
        firstDetailData.no = System.currentTimeMillis() + "";
        perReduceData.promotionDetails.add(firstDetailData);
        PerReduceView firstReduce = new PerReduceView(context, firstDetailData);
        per_reduce_container.addView(firstReduce.getRootView());
        firstReduce.setOnDelPerReduceListener(new PerReduceView.OnDelPerReduceListener() {
            @Override
    ...
    private void sendPerReduce() {
        ...
        /*showReadDialog();
        MarketPRManager.getMarketPRManager().addPerReduce(this, sharedPreferencesUtil.getString(LoginMemory.SHOP_ID), perReduceData, new MarketPRManager.OnAddPerReduceListener() {
            @Override
            public void onAddPerReduceSuccess(BaseActivity baseActivity) {
                dismissReadDialog();
                setResult(RESULT_OK);
                finish();
            }
            ...
            @Override
            public void onAddPerReduceFail(BaseActivity baseActivity, String failStr) {
                dismissReadDialog();
